...
用 Vue 3 + vue-router 搭的最小示例，用途是**反证 `@ai-native/core` 真的框架无关**：与 React demo 用的是同一个 core（`runAgent` / `executor` / `domPresenter`），只把适配层换成 `@ai-native/vue`。

## 反证了什么

- **执行链框架无关**：`useAIAgent` composable 用 Vue 的 `ref` 管状态、`vue-router` 的 `push` 做导航，内部调的仍是 core 的同一个 `runAgent`。搭这个 demo 全程**没改 core 一行**。
- **扫描链框架无关**：`@ai-native/scanner` 直接扫 `.vue` 的 `<template>` 里的 `data-ai-*`（用 `@vue/compiler-sfc`），生成的 `src/ai-manifest.json` 与 React demo 同构。
...
src/
  ...
  ai/AIBar.vue                       组合 @ai-native/vue 的 useAIAgent + AIBar
